...
- **业务SDK** `jellyfin_service`：业务逻辑组合、自有模型（MediaItem 等）、UI 组件
...
class MediaLibrary {
  ...
  final MediaLibraryType type;       // movies | tvshows | music | musicVideos | homeVideos | boxSets | unknown
...
| `logout()` | 登出 |
| `getContinueWatching(limit)` | 获取继续观看列表 |
| `updateUserItemData(itemId, playedPercentage)` | 更新播放进度 |
