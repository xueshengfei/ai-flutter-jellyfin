...
void main() {
  ...
  group('ArtistDetailPage', () {
    ...
    testWidgets('点击专辑触发 AppNavigator 导航', (tester) async {
      var navigated = false;
      ...
      final fakeNavigator = _FakeAppNavigator(
        onPushIntent: (intent) {
          navigated = true;
          if (intent is RouteNavigationIntent) {
            targetAlbumId = intent.arg<String>('albumId');
          }
        },
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: ServiceRegistry(
            services: {AppNavigator: fakeNavigator},
  ...
  group('MusicPlayerPage RVC action', () {
    testWidgets('keeps a static RVC button and opens RVC on tap', (
      ...
      final fakeNavigator = _FakeAppNavigator(
        onPushIntent: (intent) {
          if (intent is RouteNavigationIntent &&
              intent.routeName == JellyfinRouteNames.rvc) {
            opened = true;
          }
        },
      );
...
/// 用于测试的 AppNavigator 最小实现
class _FakeAppNavigator implements AppNavigator {
  final void Function(NavigationIntent intent)? onPushIntent;

  _FakeAppNavigator({this.onPushIntent});

  @override
  Future<T?> push<T>(String routeName, {Map<String, Object?>? arguments}) async =>
      null;

  @override
  Future<T?> pushIntent<T>(NavigationIntent intent) async {
    onPushIntent?.call(intent);
    return null;
  }
  ...
  @override
  Future<T?> replace<T>(String routeName, {Map<String, Object?>? arguments}) async =>
      null;
}
