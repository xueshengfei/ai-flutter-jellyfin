...
void main() {
  group('SeasonsPage', () {
    ...
    testWidgets('点击季触发导航', (WidgetTester tester) async {
      final seasons = [
        Season(
          id: 'season1',
          seriesId: 's1',
      ...
      await tester.pumpWidget(MaterialApp(
        home: ServiceRegistry(
          services: {AppNavigator: _FakeAppNavigator()},
          child: SeasonsPage(
            series: MediaItem(
                id: 's1', name: '测试剧集', type: 'series', serverUrl: 'http://localhost'),
            fetchSeasons: (_) => Future.value(
  ...
  group('EpisodesPage', () {
    ...
    testWidgets('点击播放按钮触发导航', (WidgetTester tester) async {
      final episodes = [
        Episode(
          id: 'ep1',
          seriesId: 's1',
...
class _FakeAppNavigator implements AppNavigator {
  @override
  Future<T?> push<T>(String routeName, {Map<String, Object?>? arguments}) async => null;

  @override
  Future<T?> pushIntent<T>(NavigationIntent intent) async => null;

  @override
  void pop<T extends Object?>([T? result]) {}

  @override
  Future<T?> replace<T>(String routeName, {Map<String, Object?>? arguments}) async => null;
}
