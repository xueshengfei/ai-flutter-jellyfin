...
/// 音乐 App 的个人模块 Repository 适配器
///
/// 只处理音乐类型（PersonalModuleConfig.musicOnly()）。
final class JellyfinPersonalRepositoryAdapter
  ...
  models.MediaItemListResult _mapResult(
    ...
    final typed = result as jellyfin_dart.BaseItemDtoQueryResult?;
    final items = typed?.items ?? const <jellyfin_dart.BaseItemDto>[];
    return models.MediaItemListResult(
      items: items
          .map((dto) => _mapMediaItem(dto, serverUrl))
          .toList(),
      totalCount: typed?.totalRecordCount,
      startIndex: typed?.startIndex,
    );
  }

  /// 简化版 MediaItem 映射（个人模块不需要完整字段）
