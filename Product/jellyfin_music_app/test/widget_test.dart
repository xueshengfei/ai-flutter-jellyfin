...
import 'package:jellyfin_music_app/src/session/app_session.dart';
import 'package:jellyfin_music_app/src/session/app_session_controller.dart';
import 'package:jellyfin_music_app/src/app/app_router.dart';
  ...
  group('resolveAuthRedirect', () {
    test('未登录时重定向到 /login', () {
      final result = resolveAuthRedirect(
        isLoggedIn: false,
        matchedLocation: '/music',
      );
      expect(result, '/login');
    });

    test('已登录且在 /login 时重定向到 /music', () {
      final result = resolveAuthRedirect(
        isLoggedIn: true,
